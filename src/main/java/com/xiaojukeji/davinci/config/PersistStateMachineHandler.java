...
/**
 * @description 状态机持久化Handler
 * @auther duenpu
 * @date 2021/9/26 16:34
 */
public class PersistStateMachineHandler extends LifecycleObjectSupport {

    private final StateMachine<PackageStates, PackageEvents> stateMachine;
     ...
     */
    public PersistStateMachineHandler(StateMachine<PackageStates, PackageEvents> stateMachine) {
     ...
     */
    public boolean handleEventWithState(Message<PackageEvents> event, PackageStates state) {
        stateMachine.stop();
        List<StateMachineAccess<PackageStates, PackageEvents>> withAllRegions = stateMachine.getStateMachineAccessor()
                .withAllRegions();
        for (StateMachineAccess<PackageStates, PackageEvents> a : withAllRegions) {
    ...
    public interface PersistStateChangeListener {
         ...
         * @param stateMachine 状态机实例
         */
        void onPersist(State<PackageStates, PackageEvents> state, Message<PackageEvents> message,
                Transition<PackageStates,
                        PackageEvents> transition,
                StateMachine<PackageStates, PackageEvents> stateMachine);
    }

    private class PersistingStateChangeInterceptor extends StateMachineInterceptorAdapter<PackageStates,
            PackageEvents> {

        // 状态预处理的拦截器方法
        @Override
        public void preStateChange(State<PackageStates, PackageEvents> state, Message<PackageEvents> message,
                Transition<PackageStates, PackageEvents> transition, StateMachine<PackageStates,
                PackageEvents> stateMachine) {
    ...
    private class CompositePersistStateChangeListener extends AbstractCompositeListener<PersistStateChangeListener> implements
        ...
        @Override
        public void onPersist(State<PackageStates, PackageEvents> state, Message<PackageEvents> message,
                Transition<PackageStates, PackageEvents> transition, StateMachine<PackageStates,
                PackageEvents> stateMachine) {
